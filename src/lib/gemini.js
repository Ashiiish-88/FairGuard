    ...
    generateContent: async (prompt) => {
      if (_usingVertex && _vertexModel && !_vertexAuthFailed) {
        try {
          ...
          // Distinguish auth/config errors (permanent) from transient errors (retry-able)
          const isPermanentError = e.message?.includes("GoogleAuthError") 
            || e.message?.includes("authenticate") 
            || e.constructor?.name === "GoogleAuthError"
            || e.message?.includes("PERMISSION_DENIED")
            || e.message?.includes("PermissionDenied")
            || e.message?.includes("IAM")
            || e.message?.includes("ServiceDisabled")
            || e.message?.includes("API not enabled")
            || e.message?.includes("PROJECT_INVALID")
            || e.status === 403
            || e.status === 400;

          if (isPermanentError) {
            // Auth/Config will never succeed without credential/project changes — permanently skip Vertex AI
            _vertexAuthFailed = true;
            _vertexModel = null;
