...
const DEMO_DATASETS = [
  ...
  {label: "Lending & Credit",    file: "/demo_lending_data.csv",       type: "csv",},
];
    ...
    const processData = async (parsedData) => {
      ...
      try {
        ...
        setConfig((prev) => {
          ...
          const positiveKeywords = ["1", "true", "hired", "approved", "yes", "flagged", "removed", "selected", "granted", "allowed", "accepted"];
          const uniqueVals = (outcomeCol?.unique_values || []).map(String);
          const guessedPositive = uniqueVals.find(v => positiveKeywords.includes(v.toLowerCase())) ?? uniqueVals[0] ?? "1";
          return {
            ...prev,
                      ...
                      <div className="flex flex-col gap-2 pt-4 border-t border-border">
                        ...
                        {config.outcome && config.positiveOutcome && data && (() => {
                          ...
                          return (
                            <div
                              className={[
                                "flex items-center gap-2 px-3 py-2 rounded-md text-xs border mt-1",
                                isZero
